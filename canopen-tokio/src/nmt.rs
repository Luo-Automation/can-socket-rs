...
/// Send an NMT command.
pub async fn send_nmt_command(bus: &CanOpenSocket, node_id: u8, command: NmtCommand) -> Result<(), NmtError> {
...
pub async fn send_nmt_command_timeout(bus: &CanOpenSocket, node_id: u8, command: NmtCommand, timeout: Duration) -> Result<(), NmtError> {
